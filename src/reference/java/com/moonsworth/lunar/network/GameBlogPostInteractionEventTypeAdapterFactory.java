...
public class GameBlogPostInteractionEventTypeAdapterFactory implements TypeAdapterFactory {
   public GameBlogPostInteractionEventTypeAdapterFactory() {
   }

   public <T> TypeAdapter<T> create(Gson gson1, TypeToken<T> typetoken2) {
      if (!NetworkIterator.class.isAssignableFrom(typetoken2.getRawType())) {
         return null;
      }
