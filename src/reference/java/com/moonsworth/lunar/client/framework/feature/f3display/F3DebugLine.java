...
public enum F3DebugLine implements com.moonsworth.lunar.client.config.option.OptionEnumValue {
   ...
   F3DebugLine(String text3, String text4) {
   ...
   public static F3DebugLine getFromId(String text0) {
      return (F3DebugLine)F3Display.extensionById.get(text0);
   }

   public static List<String> ids() {
      return Arrays.stream(values()).map(F3DebugLine::id).toList();
   }

   public boolean isAllowed(boolean flag1, boolean flag2) {
      if (!flag1) {
         return true;
      } else {
         return this == PIE_CHART && flag2 ? true : this.allowedInReducedInfo;
      }
   }

   @Generated
   F3DebugLine(String text3, String text4, boolean flag5) {
      this.id = text3;
      this.display = text4;
      this.allowedInReducedInfo = flag5;
   }
