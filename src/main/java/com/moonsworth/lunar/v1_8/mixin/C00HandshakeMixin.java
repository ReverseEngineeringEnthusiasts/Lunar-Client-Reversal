...
public class C00HandshakeMixin {
   ...
   public void readPacketData(PacketBuffer packetbuffer1) {
      this.protocolVersion = packetbuffer1.readVarIntFromBuffer();
      this.ip = packetbuffer1.readStringFromBuffer(32767);
      this.port = packetbuffer1.readUnsignedShort();
      this.requestedState = EnumConnectionState.getById(packetbuffer1.readVarIntFromBuffer());
   }

   @Overwrite
   public void writePacketData(PacketBuffer packetbuffer1) {
      packetbuffer1.writeVarIntToBuffer(this.protocolVersion);
      packetbuffer1.writeString(this.ip);
      packetbuffer1.writeShort(this.port);
      packetbuffer1.writeVarIntToBuffer(this.requestedState.getId());
   }
}
