...
public abstract class StructureComponent
{
    ...
    protected void func_175805_a(World worldIn, StructureBoundingBox boundingboxIn, Random rand, float chance, int minX, int minY, int minZ, int maxX, int maxY, int maxZ, IBlockState state, IBlockState state2, boolean p_175805_13_)
    {
        for (int i = minY; i <= maxY; ++i)
        {
            for (int j = minX; j <= maxX; ++j)
            {
                for (int k = minZ; k <= maxZ; ++k)
                {
                    if (rand.nextFloat() <= chance && (!p_175805_13_ || this.getBlockStateFromPos(worldIn, j, i, k, boundingboxIn).getBlock().getMaterial() != Material.air))
                    {
                        ...
                        {
                            this.setBlockState(worldIn, state2, j, i, k, boundingboxIn);
                        }
                        else
                        {
                            this.setBlockState(worldIn, state, j, i, k, boundingboxIn);
                        }
                    }
                }
            }
